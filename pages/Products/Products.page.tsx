...
import { authors, physicalProducts, digitalProducts, lmsProducts, leadProducts } from '../../constants/products';
...
const ProductsPage: React.FC<ProductsPageProps> = ({ setCurrentPage }) => {
    const [activeTab, setActiveTab] = useState<ProductCategory>(ProductCategory.Fisik);
    const [searchTerm, setSearchTerm] = useState('');

    const processedData = useMemo(() => {
        const createProductData = (product: { name: string; price: number }, type: string): Product => {
            ...
            return { id: slug, name: product.name, price: product.price, orders, paid, paidRatio, revenue, author, lastChanged, url };
        };
        ...
        return {
            [ProductCategory.Fisik]: physicalProducts.map(p => createProductData(p, 'fisik')),
            [ProductCategory.Digital]: digitalProducts.map(p => createProductData(p, 'digital')),
            [ProductCategory.LMS]: lmsProducts.map(p => createProductData(p, 'lms')),
    ...
    return (
         <div className="flex-1 flex flex-col overflow-hidden">
            ...
            <ActionBar searchTerm={searchTerm} setSearchTerm={setSearchTerm} onAddProduct={() => setCurrentPage('product-creation')} />
