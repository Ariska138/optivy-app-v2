...
export const MobileBottomNav: React.FC<{
    ...
    return (
        <div className="md:hidden fixed bottom-0 left-0 right-0 bg-sidebar-bg border-t border-border-color h-16 flex justify-around items-center z-40">
            <button
                onClick={onAddComponentClick}
                className={`flex flex-col items-center gap-1 text-xs font-medium ${activePanel === 'components' ? 'text-accent' : 'text-text-tertiary'}`}
            ...
            <button
                ...
                className={`flex flex-col items-center gap-1 text-xs font-medium disabled:text-gray-300 ${activePanel === 'properties' ? 'text-accent' : 'text-text-tertiary'}`}
