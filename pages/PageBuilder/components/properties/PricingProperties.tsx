...
export const PricingProperties: React.FC<EditorProps<ComponentPropsMap['Pricing']>> = ({ props, update, markUnsaved }) => {
    ...
    return (
        <>
            ...
            <div className="flex items-center justify-between p-2 rounded-md bg-item-bg my-3">
                <label htmlFor="popular-toggle" className="text-sm font-medium text-text-secondary">
                    Mark as Popular
                </label>
                <input
                    id="popular-toggle"
            ...
            <div>
                ...
                <button onClick={handleAddFeature} className="text-sm text-blue-600 hover:underline flex items-center gap-1"><Plus size={14} /> Add Feature</button>
            </div>
        </>
    );
};