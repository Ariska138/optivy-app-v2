...
const RoleBadge: React.FC<{ role: TeamRole }> = ({ role }) => {
    ...
    return (
        <span className={`px-2.5 py-0.5 text-xs font-medium rounded-full ${styles[role]}`}>
            {role}
        </span>
    );
};

const MemberItem: React.FC<{ member: ManagedTeamMember, onDelete: (id: number) => void }> = ({ member, onDelete }) => (
    <div className="flex items-center justify-between p-4 transition-colors hover:bg-violet-50">
        ...
        <div className="flex items-center gap-4">
            ...
            <button onClick={() => onDelete(member.id)} className="text-gray-400 hover:text-red-600 p-1">
    ...
    return (
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200">
            <div className="p-4 border-b">
                <h3 className="font-semibold text-lg text-gray-800">Anggota Tim ({members.length})</h3>
            </div>
            <div className="divide-y divide-gray-200">
                {members.map(member => <MemberItem key={member.id} member={member} onDelete={onDelete} />)}
            </div>
        </div>
    );
};