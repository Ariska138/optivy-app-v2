...
const ProductSummary: React.FC<{ data: Product[]; categoryName: string }> = ({
  data,
  categoryName,
}) => {
  ...
  return (
    <div className="summary-slider flex overflow-x-auto py-2 space-x-4">
      <SummaryCard
        title={`Total Produk ${categoryName}`}
        value={totalProducts}
      />
      <SummaryCard
...
const LeadSummary: React.FC<{ data: LeadProduct[] }> = ({ data }) => {
  ...
  return (
    <div className="summary-slider flex overflow-x-auto py-2 space-x-4">
      ...
      <SummaryCard
        title="Total Prospek"
        value={totalProspects.toLocaleString('id-ID')}
        color="text-blue-600"
      />
    </div>
  );
  ...
  const renderSummary = () => {
    switch (activeTab) {
      ...
      case ProductCategory.Digital:
        return (
          <ProductSummary data={data as Product[]} categoryName="Digital" />
        );
