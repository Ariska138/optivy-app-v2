...
const SummaryCard: React.FC<{
  ...
  value: string | number;
  color?: string;
}> = ({ title, value, color = 'text-violet-600' }) => (
...
const DigitalSummary: React.FC<{ data: Order[] }> = ({ data }) => {
  ...
  return (
    <div className="summary-slider flex overflow-x-auto py-2 space-x-4">
      ...
      <SummaryCard
        title="Bump Revenue"
        value={formatCurrency(grossRevenue * 0.12)}
      />
      ...
      <SummaryCard
        title="Net Revenue"
        value={formatCurrency(grossRevenue * 0.95)}
        color="text-green-600"
      />
      <SummaryCard
        title="Gross Profit"
        value={formatCurrency(grossRevenue * 0.7)}
        color="text-green-600"
      />
      <SummaryCard
        title="Net Profit"
        value={formatCurrency(grossRevenue * 0.55)}
        color="text-green-600"
      />
    </div>
  );
...
const FisikSummary: React.FC<{ data: Order[] }> = ({ data }) => {
  ...
  return (
    <div className="summary-slider flex overflow-x-auto py-2 space-x-4">
      ...
      <SummaryCard
        title="Bump Revenue"
        value={formatCurrency(grossRevenue * 0.1)}
      />
      <SummaryCard
        title="Discount"
        value={formatCurrency(grossRevenue * 0.05)}
      />
      <SummaryCard
        title="Shipping Cost"
        value={formatCurrency(data.length * 15000)}
      />
      ...
      <SummaryCard
        title="Net Revenue"
        value={formatCurrency(grossRevenue * 0.9)}
        color="text-green-600"
      />
      <SummaryCard
        title="Gross Profit"
        value={formatCurrency(grossRevenue * 0.6)}
        color="text-green-600"
      />
      <SummaryCard
        title="Expenses"
        value={formatCurrency(grossRevenue * 0.15)}
        color="text-red-500"
      />
      <SummaryCard
        title="Net Profit"
        value={formatCurrency(grossRevenue * 0.45)}
        color="text-green-600"
      />
    </div>
  );
...
export const SummaryPanel: React.FC<SummaryPanelProps> = ({
  ...
  return <div className="p-4 bg-violet-50">{renderSummary()}</div>;
};
