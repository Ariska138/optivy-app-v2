    ...
    <div className="mt-4 pt-4 border-t">
      ...
      <div className="flex justify-between items-center gap-2">
        <span className="font-mono text-sm text-gray-700 truncate">
          2405:8180:401:1eac:3d9d:69a9:312:a6f6
        </span>
        <button className="text-xs text-red-600 border border-red-300 rounded-md px-2 py-1 hover:bg-red-50 flex-shrink-0">
          Blokir IP
        </button>
      </div>
    </div>
  ...
  <DetailCard>
    ...
    <div className="space-y-3 text-sm">
      ...
      <div>
        ...
        <pre className="bg-gray-100 text-gray-600 text-xs p-3 mt-1 rounded-lg overflow-x-auto">
          <code>{order.utm?.split('_').join('\n') || 'N/A'}</code>
        </pre>
      </div>
    </div>
  </DetailCard>
...
const PaymentMethodCard: React.FC<{ order: Order }> = ({ order }) => (
  <DetailCard>
    ...
    <div className="flex items-center gap-3">
      <img
        src={`https://placehold.co/40x40/random/FFFFFF?text=${order.paymentMethod.substring(
          0,
          3
        )}`}
...
const OrderSummaryCard: React.FC<{ viewType: ViewType; order: Order }> = ({
  viewType,
  order,
}) => {
      ...
      <div className="space-y-2 text-sm text-gray-600">
        {isSubscription ? (
          <div className="flex justify-between">
            <span>Harga Langganan</span> <span>Rp {format(total)}</span>
          </div>
        ) : (
          <div className="flex justify-between">
            <span>Subtotal Produk</span> <span>Rp {format(subtotal)}</span>
          </div>
        )}
        {shipping > 0 && (
          <div className="flex justify-between">
            <span>Biaya Pengiriman</span> <span>Rp {format(shipping)}</span>
          </div>
        )}
        {discount > 0 && (
          <div className="flex justify-between">
            <span>Diskon</span>{' '}
            <span className="text-green-600">- Rp {format(discount)}</span>
          </div>
        )}
  ...
  const historyData = {
    [ViewType.Physical]: [
      {
        ...
        content: (
          <>
            Status: <span className="font-bold">Dalam Pengiriman</span>.{' '}
            <span className="text-gray-400">(11/08)</span>
          </>
        ),
      },
      {
        ...
        content: (
          <>
            Status: <span className="font-bold text-green-600">Lunas</span>.{' '}
            <span className="text-gray-400">(10/08)</span>
          </>
        ),
      },
      {
        ...
        content: (
          <>
            Email <span className="font-bold">Info Pengiriman</span> dikirim.{' '}
            <span className="text-gray-400">(11/08)</span>
          </>
        ),
      },
    ...
    [ViewType.Digital]: [
      {
        ...
        content: (
          <>
            Status: <span className="font-bold">Selesai</span>.{' '}
            <span className="text-gray-400">(10/08)</span>
          </>
        ),
      },
      ...
      },
      {
        ...
        content: (
          <>
            Email <span className="font-bold">Akses Produk</span> dikirim.{' '}
            <span className="text-gray-400">(10/08)</span>
          </>
        ),
      },
    ...
    [ViewType.LMS]: [
      ...
      },
      {
        ...
        content: (
          <>
            Email <span className="font-bold">Akses LMS</span> dikirim.{' '}
            <span className="text-gray-400">(10/08)</span>
          </>
        ),
      },
    ...
    [ViewType.Lead]: [
      {
        ...
        content: (
          <>
            Status: <span className="font-bold">Baru</span>.{' '}
            <span className="text-gray-400">(10/08)</span>
          </>
        ),
      },
      {
        ...
        content: (
          <>
            Email <span className="font-bold">Konfirmasi Lead</span> dikirim.{' '}
            <span className="text-gray-400">(10/08)</span>
          </>
        ),
      },
...
export const OrderDetailSidebar: React.FC<SidebarProps> = ({
  viewType,
  order,
}) => {
  ...
  return (
    <aside className="lg:col-span-1 space-y-6">
      ...
      {viewType !== ViewType.Lead && (
        <PaymentMethodCard order={order as Order} />
      )}
      {viewType !== ViewType.Lead && (
        <OrderSummaryCard viewType={viewType} order={order as Order} />
      )}
