...
const CheckoutPreview: React.FC<CheckoutPreviewProps> = ({ formData }) => {
  ...
  if (isUniqueCodeActive) {
    if (checkout.uniqueCodeOperator === 'increase')
      totalPrice += uniqueCodeValue;
      ...
      <div className="bg-white p-6 rounded-xl shadow-inner space-y-5">
        <div className="text-center">
          <h3 className="text-2xl font-bold text-slate-900">
            {checkout.tagline || 'Judul & Tagline Anda'}
          </h3>
          {checkout.subheadline && (
            <p className="text-slate-500 mt-1">{checkout.subheadline}</p>
          )}
        </div>
            ...
            {checkout.formFields
              ...
              .map((field) => (
                <div key={field.id} className="space-y-1">
                  ...
                  <input
                    type="text"
                    className="bg-slate-50 border border-slate-200 rounded-lg p-3 w-full"
        ...
        {(checkout.paymentMethods.bankTransfer ||
          checkout.paymentMethods.ePayment) && (
          <div className="pt-4 border-t border-slate-100">
            ...
            <select className="bg-slate-50 border border-slate-200 rounded-lg p-3 w-full">
              {checkout.paymentMethods.bankTransfer && (
                <option>Bank Transfer (BCA, Mandiri, BNI)</option>
              )}
              {checkout.paymentMethods.ePayment && (
        ...
        {checkout.showSummaryOrder && (
          <div className="p-4 rounded-lg bg-slate-50 space-y-2">
            ...
            <div className="flex justify-between items-center font-bold">
              <span className="text-purple-600">Harga Spesial</span>
            ...
            {isUniqueCodeActive && (
              <div className="flex justify-between items-center text-sm">
                ...
                <span>
                  {checkout.uniqueCodeOperator === 'increase' ? '+' : '-'}{' '}
        ...
        <button
          ...
          style={{ backgroundColor: checkout.buttonColor }}
        >
          {checkout.buttonText}
        </button>

        <div className="flex justify-center items-center gap-4 pt-3 text-slate-500">
          {checkout.trustSymbols.guarantee && (
            <div className="flex items-center gap-2 text-xs">
              <i className="fa-solid fa-shield-halved"></i>
              <span>Garansi 30 Hari</span>
            </div>
          )}
          {checkout.trustSymbols.securePayment && (
            <div className="flex items-center gap-2 text-xs">
              <i className="fa-solid fa-lock"></i>
              <span>Pembayaran Aman</span>
            </div>
          )}
          {checkout.trustSymbols.ssl && (
            <div className="flex items-center gap-2 text-xs">
        ...
        {checkout.showOfficialLogo && (
          <div className="text-center pt-4 mt-4 border-t border-slate-100">
            <img
              src="https://placehold.co/150x50/a379d7/ffffff?text=LOGO+ANDA"
              alt="Official Logo"
